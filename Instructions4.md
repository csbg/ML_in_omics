...
3. Run a loop with 5 iterations. Within the loop:
    ...
    2. Fit both models on the training data. Note: You do not have to re-define the model each time. Calling `fit()` will overwrite the previously trained weights in `sklearn` models (unless you set `warm_start = True`)
